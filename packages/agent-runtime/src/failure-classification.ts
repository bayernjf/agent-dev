...
const FAILURE_PATTERNS: FailurePattern[] = [
  ...
  {
    pattern: /(command not found|not found on local PATH|not installed|not detected).*(codex|claude|opencode|agent)|(codex|claude|opencode|agent).*(command not found|not found on local PATH|not installed|not detected)/i,
    category: 'environment',
    severity: 'blocking',
  ...
  // --- Product: Production branch mismatch ---
  {
    pattern: /(production|main).*(branch|commit).*(not found|not ancestor|missing|behind)|accepted commit.*not.*(ancestor|main)|dev.*main.*merge/i,
    category: 'product',
    severity: 'blocking',
    title: 'Production branch does not contain the approved commit',
    explanation: 'The production release requires the approved commit to be on the production branch (main). The approved commit is only on the dev branch or a feature branch. This prevents releasing from the production branch.',
    remediation: [
      'Merge the dev branch (or feature branch) into main',
      'Create a pull request from dev to main and merge it',
      'After merging, retry the production release',
      'Note: Agent-Dev never auto-merges dev into main — this is a deliberate human gate',
    ],
    autoRetryable: false,
    relatedDefects: [9],
  },
  // --- Environment: Workspace corrupted / needs recovery ---
  {
    pattern: /(workspace).*(corrupt|broken|stale|missing|unusable|not usable|dirty)|staleConfig|workspace.*recover/i,
    category: 'environment',
    severity: 'recoverable',
    title: 'Workspace needs recovery',
    explanation: 'The isolated workspace for this project is in an inconsistent state. This can happen if a previous agent run was interrupted, or if the workspace was generated by an older version of the generator.',
    remediation: [
      'Use the "Recover Workspace" button in the Studio UI (Delivery tab)',
      'Recovery creates a fresh workspace and preserves the old one for inspection',
      'After recovery, re-run the Apply step to regenerate the workspace',
      'If recovery is not available, delete the local .agent-dev directory and re-apply',
    ],
    autoRetryable: false,
    relatedDefects: [17, 18],
  },
  // --- Configuration: Schema migration / old data ---
  {
    pattern: /(schema|migration|parse|validation).*(fail|error|missing|default)|ZodError|safeParse.*fail/i,
    category: 'configuration',
    severity: 'recoverable',
    title: 'Data schema migration needed',
    explanation: 'Stored project data could not be parsed with the current schema. This typically happens after an Agent-Dev upgrade that added new required fields to the Blueprint or project data model.',
    remediation: [
      'This is usually handled automatically by the system (missing fields get default values)',
      'If the error persists, try re-saving the Blueprint in Studio to trigger a re-write with the current schema',
      'For very old projects, you may need to recreate the Blueprint from scratch',
      'Check the daemon logs for the specific field that is missing',
    ],
    autoRetryable: false,
    relatedDefects: [25],
  },
  // --- Configuration: Cloud resource mismatch ---
  {
    pattern: /(provider|cloud|resource).*(mismatch|not found|not configured|missing)|deployment.*target.*(none|missing|not exist)/i,
    category: 'configuration',
    severity: 'blocking',
    title: 'Cloud resource configuration mismatch',
    ...
    autoRetryable: false,
    relatedDefects: [24, 26],
  },
  // --- Configuration: External symlink blocked ---
  {
    pattern: /(symlink|symbolic link|external link).*(block|outside|workspace|escape|not allowed)/i,
    category: 'configuration',
    severity: 'blocking',
    title: 'External symlink blocked by safety guard',
    explanation: 'Agent-Dev blocked a commit because it detected a symbolic link pointing outside the project workspace. This is a safety guard to prevent the agent from linking to external directories (like node_modules from another project) that would be committed as broken links.',
    remediation: [
      'Check the workspace for symbolic links pointing outside the project directory',
      'Remove the external symlink and install dependencies properly within the project',
      'Ensure .gitignore covers dependency directories (node_modules, .venv, etc.)',
      'After cleaning up, retry the operation',
    ],
    autoRetryable: false,
    relatedDefects: [7],
  ...
  {
    pattern: /(npm install|npm ci|dependency|dependencies).*(fail|error|ETARGET|ERESOLVE|peer dep)|node_modules.*missing/i,
    category: 'environment',
    severity: 'recoverable',
    title: 'Dependency installation failed',
    explanation: 'Project dependencies could not be installed. This may be due to network issues, incompatible Node versions, or conflicting peer dependencies.',
    remediation: [
      'Ensure you are using Node.js 22 or newer (run `node --version`)',
      'Delete node_modules and package-lock.json, then run `npm install` fresh',
      'If peer dependency conflicts occur, try `npm install --legacy-peer-deps`',
