...
const BUILT_IN_AGENTS: Omit<AgentDescriptor, 'detected' | 'version' | 'detail'>[] = [
...
export function discoverAgentRuntimes(custom: CustomAgentInput[] = []): AgentDescriptor[] {
  const builtIns = BUILT_IN_AGENTS.map(agent => {
    ...
    return { ...agent, ...result, detail: result.detected ? 'Detected on local PATH.' : `Install ${agent.launchCommand} or add a custom launch command.` };
  });
  const customAgents = custom.map((agent, index) => {
    const result = detect(agent.launchCommand);
    return { id: `custom-${index + 1}`, ...agent, source: 'custom' as const, ...result, detail: result.detected ? 'Custom command detected on local PATH.' : 'Command not detected. Check the launch command.' };
  });
