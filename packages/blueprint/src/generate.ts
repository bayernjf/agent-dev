...
export function generateArtifacts(blueprint: ProductBlueprint): GeneratedArtifact[] {
  ...
  artifacts.push(
    { id: 'template-root-package', title: 'Web SaaS workspace package', path: 'package.json', content: JSON.stringify({ name: blueprint.metadata.name, private: true, type: 'module', scripts: { dev: 'concurrently -k "npm run dev -w web" "npm run dev -w api"', typecheck: 'tsc --noEmit', build: 'npm run build -w web', quality: 'npm run typecheck && npm run build' }, workspaces: ['apps/web', 'apps/api'], devDependencies: { '@types/node': '^22.15.3', '@types/react': '^19.1.2', '@types/react-dom': '^19.1.2', '@vitejs/plugin-react': '^4.4.1', concurrently: '^9.1.0', typescript: '^5.8.3', vite: '^7.3.6' } }, null, 2) + '\n' },
