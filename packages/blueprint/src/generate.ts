...
export type GeneratedArtifact = {
  id: 'product-standard' | 'agent-instructions' | 'delivery-workflow' | 'environment-contract' | 'delivery-handoff' | 'template-root-package' | 'template-readme' | 'template-web-package' | 'template-web-index' | 'template-web-main' | 'template-web-styles' | 'template-api-package' | 'template-api-index' | 'template-api-vercel' | 'template-cloudflare' | 'template-quality-workflow';
  title: string;
  path: string;
  content: string;
};
...
export function generateArtifacts(blueprint: ProductBlueprint): GeneratedArtifact[] {
  ...
  artifacts.push(
    { id: 'template-root-package', title: 'Web SaaS workspace package', path: 'package.json', content: JSON.stringify({ name: blueprint.metadata.name, private: true, type: 'module', scripts: { dev: 'concurrently -k "npm run dev -w web" "npm run dev -w api"', typecheck: 'tsc --noEmit', build: 'npm run build -w web', quality: 'npm run typecheck && npm run build' }, workspaces: ['apps/web', 'apps/api'], devDependencies: { concurrently: '^9.1.0', typescript: '^5.8.3' } }, null, 2) + '\n' },
