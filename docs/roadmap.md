...
- **多 Profile 串行流水线**（一个 Feature Task 内定义多步骤，每步用不同 Profile，前步输出作为后步输入，如"方案→实现→审查"闭环；已实现，含步骤编辑器、执行进度、人工 Gate 和失败处理）；
...
> 详细拆解（优先级、代码现状对照、里程碑、风险）：[v0.2 实施计划](implementation-plan-v0.2.md)
