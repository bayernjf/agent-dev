...
- **已完成**：适配器按官方 OpenAPI 实现 v4 端点（2026-09-01 确认请求形状）；凭证系统后端化集成；19 例适配器单测（mock runner + stub fetch）+ 5 例凭证后端路由测试（含「禁止静默回退」磁盘哨兵证据）+ 1 例 daemon 契约测试 + 4 例 Studio 后端状态渲染测试，全仓 249 例全绿。
