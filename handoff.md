...
| 当前本地能力 | Blueprint Revision、Dry Run、Connector Preflight/Discovery、资源归属计划、本地审批、带合法 npm 包名和 TypeScript/Vite 质量配置的固定 Web SaaS 模板、隔离工作区 Git baseline 与本地 feature branch、Local Quality Gate、可恢复/可重试的 Local Apply Simulator、XState 状态推进、统一 Delivery Report、Fake Provider Adapter 计划/Apply/Verify 及 Studio 展示 |
